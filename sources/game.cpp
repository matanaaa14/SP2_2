...
using namespace std;
    void Game::playTurn(){
        if(this->player1.name == this->player2.name){
            throw runtime_error("can't play against himself");
        }
        this->numOfRounds+=1;
    ...
    Game::Game(Player& p1,Player& p2) : player1(p1),player2(p2){
        ...
        unsigned long randomSeed = (unsigned long) rand() %100;
        std::shuffle(v.begin(),v.end(),default_random_engine(randomSeed));
